...
int main(int argc,char **args)
{
  ...
  ierr = VecDuplicate(x,&u);CHKERRQ(ierr);    /*将x的格式赋给u*/
  ...
  ierr = VecSet(u,zero);CHKERRQ(ierr);    /*设置初始向量u*/
  if(rank == 0){    /*开始设置初始条件*/
    for(ii = 1; ii < n; ii++){    /*除首尾两个点外的其余点*/
      PetscReal inp;    /*临时参数*/
      xi = ii*dx*pi;    /*获取当前的位置，并将其乘以pi*/
      inp = dt*sin(xi);    /*根据当前位置来获取传热值*/
      ierr = VecSetValues(u, 1, &ii, &inp, INSERT_VALUES);CHKERRQ(ierr);    /*将向量的对应位置的值进行修改*/
    }
  }
  
  ierr = VecAssemblyBegin(u);CHKERRQ(ierr);    /*通知其余并行块将向量统一*/
  ierr = VecAssemblyEnd(u);CHKERRQ(ierr);    /*结束通知*/

  ierr = VecView(u,PETSC_VIEWER_STDOUT_WORLD);CHKERRQ(ierr);    /*打印向量u，检查是否出错*/
  ...
  while(PetscAbsReal(t)<3.0){    /*计算0-2时间内的传播*/
    ...
    ierr = VecCopy(x,b);CHKERRQ(ierr);    /*将x的值赋给b*/
  }

  ierr = VecView(b,PETSC_VIEWER_STDOUT_WORLD);CHKERRQ(ierr);    /*打印向量，获得结束时隐式方法的值*/
 
  ierr = VecDestroy(&x);CHKERRQ(ierr);    /*关闭向量x*/
  ierr = VecDestroy(&u);CHKERRQ(ierr);    /*关闭向量u*/
